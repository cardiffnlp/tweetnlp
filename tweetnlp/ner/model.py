    ...
    def predict(self,
        ...
        with torch.no_grad():
            for i in range(len(_index) - 1):
                ...
                pred = pred.cpu().detach().int().tolist()
                if self.crf_layer is not None:
                    if self.parallel:
                        best_path = self.crf_layer.module.viterbi_tags(output['logits'])
                    else:
                        best_path = self.crf_layer.viterbi_tags(output['logits'])
                    pred = [tag_seq for tag_seq, _ in best_path]
                probs += prob
                preds += pred
        preds = [[self.id_to_label[__p] for __p in _p] for _p in preds]
        output = {
            'prediction': preds,
            'probability': probs,
            'input': inputs,
            'entity_prediction': [
                decode_ner_tags(_p, _i, _prob) for _p, _prob, _i in zip(preds, probs, inputs)
            ]
        }
