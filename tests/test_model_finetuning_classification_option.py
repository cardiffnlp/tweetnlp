...
model_alias = f'{os.path.basename(language_model)}-{task}-single-all'
...
trainer.evaluate()
trainer.push_to_hub(hf_organization='cardiffnlp', model_alias=model_alias)
pred = trainer.predict("Jacob Collier is a Grammy-awarded English artist from London.")
pprint(pred)
pred = trainer.predict("Jacob Collier is a Grammy-awarded English artist from London.", return_probability=True)
pprint(pred)
...
task = "topic_classification"
language_model = 'distilbert-base-uncased'
model_alias = f'{os.path.basename(language_model)}-{task}-multi-all'
